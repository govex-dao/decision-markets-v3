#[test_only]
module futarchy_markets_primitives::conditional_1;

use sui::coin_registry::{Self, CoinRegistry};

/// Marker type for CONDITIONAL_1 (needs key ability for non-OTW new_currency pattern)
public struct CONDITIONAL_1 has key { id: UID }
...
public fun init_for_testing(registry: &mut CoinRegistry, ctx: &mut TxContext) {
    let (initializer, treasury_cap) = coin_registry::new_currency<CONDITIONAL_1>(
        registry,
