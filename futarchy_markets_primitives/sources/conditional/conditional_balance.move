...
// === Constants ===
const VERSION: u8 = 1;
// MAX_OUTCOMES is module-specific (200), intentionally higher than protocol_max_outcomes (50)
// because conditional_balance is a low-level structure that could support more outcomes than governance allows
...
public fun new<AssetType, StableType>(
    ...
    assert!(outcome_count >= (constants::min_outcomes() as u8), EInvalidOutcomeCount);
...
public(package) fun set_balance<AssetType, StableType>(
...
/// Add to a single balance slot (outcome + type).
///
/// Quantum operations that affect *all* outcomes (split/recombine) perform the
/// cross-outcome loops in their own entrypoints; this helper mutates one slot.
public fun add_to_balance<AssetType, StableType>(
    ...
    add_to_balance_pkg(balance, outcome_idx, is_asset, amount);
}

/// Package-only raw mutator used by trusted internal flows (wrap/unwrap/burn/etc).
public(package) fun add_to_balance_pkg<AssetType, StableType>(
    balance: &mut ConditionalMarketBalance<AssetType, StableType>,
    outcome_idx: u8,
    is_asset: bool,
    amount: u64,
    ...
    // Explicit overflow check with clear error message
    assert!(*current <= 18446744073709551615u64 - amount, EBalanceOverflow);
...
public fun sub_from_balance<AssetType, StableType>(
    ...
    sub_from_balance_pkg(balance, outcome_idx, is_asset, amount);
}

/// Package-only raw mutator used by trusted internal flows (wrap/unwrap/burn/etc).
public(package) fun sub_from_balance_pkg<AssetType, StableType>(
    balance: &mut ConditionalMarketBalance<AssetType, StableType>,
    outcome_idx: u8,
    is_asset: bool,
    amount: u64,
...
public fun merge<AssetType, StableType>(
    ...
    while (i < len) {
        ...
        if (src_val > 0) {
            ...
            assert!(dest_val <= 18446744073709551615u64 - src_val, EBalanceOverflow);
...
///
/// This converts a specified amount from balance into a real typed Coin that can be used
/// in external DeFi protocols or traded on DEXes. Supports partial unwrapping.
///
/// # Security
...
/// * `amount` - Amount to unwrap (supports partial unwrapping)
///
/// # Returns
...
/// * `EInsufficientBalance` - If balance is less than requested amount
/// * `EInvalidBalanceAccess` - If amount is zero
///
/// # Example
/// ```move
/// // User has outcome 0 asset balance of 1000, wants to unwrap 500
...
///     ctx
/// );
/// // Now user has a Coin<Cond0Asset> worth 500, balance still has 500
/// ```
public fun unwrap_to_coin<AssetType, StableType, ConditionalCoinType>(
    ...
    let current_balance = get_balance(balance, outcome_idx, is_asset);
    assert!(current_balance >= amount, EInsufficientBalance);

    // Decrement wrapped balance tracking before minting
    coin_escrow::decrement_wrapped_balance_pkg(escrow, (outcome_idx as u64), is_asset, amount);

    // CORRECT ORDER: Mint first, then subtract balance
    // This ensures if minting fails, balance isn't lost
    let coin = coin_escrow::mint_conditional_pkg<AssetType, StableType, ConditionalCoinType>(
        escrow,
        (outcome_idx as u64),
        is_asset,
        amount,
    ...
    // Subtract from balance (supports partial unwrap)
    sub_from_balance_pkg(balance, outcome_idx, is_asset, amount);
...
public fun wrap_coin<AssetType, StableType, ConditionalCoinType>(
    ...
    coin_escrow::burn_conditional_pkg<AssetType, StableType, ConditionalCoinType>(
        escrow,
        (outcome_idx as u64),
        is_asset,
        coin,
    ...
    coin_escrow::increment_wrapped_balance_pkg(escrow, (outcome_idx as u64), is_asset, amount);
...
public fun burn_complete_set_and_withdraw_from_balance<AssetType, StableType>(
    ...
    assert!((balance.outcome_count as u64) == coin_escrow::caps_registered_count(escrow), EInvalidOutcomeCount);
    ...
    // CRITICAL: Decrement outcome_escrowed for ALL outcomes to maintain quantum invariant
    // Invariant: outcome_escrowed[i] == supply[i] + wrapped[i]
    // Since wrapped[i] will decrease below, outcome_escrowed[i] must also decrease
    coin_escrow::decrement_outcome_escrowed_for_all(escrow, is_asset, min_amount);

    // Subtract min_amount from each outcome and decrement wrapped balance tracking
    i = 0;
    while (i < (outcome_count as u64)) {
        sub_from_balance_pkg(balance, (i as u8), is_asset, min_amount);
        // Decrement wrapped balance tracking for quantum invariant
        coin_escrow::decrement_wrapped_balance_pkg(escrow, i, is_asset, min_amount);
        i = i + 1;
    };

    // Withdraw from escrow and decrement user backing
    if (is_asset) {
        let asset_coin = coin_escrow::withdraw_asset_balance_pkg(escrow, min_amount, ctx);
        coin_escrow::decrement_user_backing_pkg(escrow, min_amount, true);

        // Verify quantum invariant after all updates
        coin_escrow::assert_quantum_invariant(escrow);

        (min_amount, asset_coin, sui::coin::zero<StableType>(ctx))
    } else {
        let stable_coin = coin_escrow::withdraw_stable_balance_pkg(escrow, min_amount, ctx);
        coin_escrow::decrement_user_backing_pkg(escrow, min_amount, false);
...
public fun split_stable_to_balance<AssetType, StableType>(
    ...
    // CRITICAL: Increment outcome_escrowed for ALL outcomes to maintain quantum invariant
    // Invariant: outcome_escrowed[i] == supply[i] + wrapped[i]
    // Since wrapped[i] will increase below, outcome_escrowed[i] must also increase
    coin_escrow::increment_outcome_escrowed_for_all(escrow, false, amount);
    ...
    let mut i = 0u64;
    while (i < (outcome_count as u64)) {
        // Increment wrapped for this outcome
        coin_escrow::increment_wrapped_balance_pkg(escrow, i, false, amount);

        // Update balance tracker
        add_to_balance_pkg(balance, (i as u8), false, amount);
...
public fun split_asset_to_balance<AssetType, StableType>(
    ...
    // Invariant: outcome_escrowed[i] == supply[i] + wrapped[i]
    // Since wrapped[i] will increase below, outcome_escrowed[i] must also increase
    coin_escrow::increment_outcome_escrowed_for_all(escrow, true, amount);

    // Quantum split: increment wrapped for ALL outcomes
    // (no supply increment - goes directly to balance form, not typed coin)
    let outcome_count = balance.outcome_count;
    let mut i = 0u64;
    while (i < (outcome_count as u64)) {
        // Increment wrapped for this outcome
        coin_escrow::increment_wrapped_balance_pkg(escrow, i, true, amount);

        // Update balance tracker
        add_to_balance_pkg(balance, (i as u8), true, amount);
...
public fun recombine_balance_to_stable<AssetType, StableType>(
    ...
    // Invariant: outcome_escrowed[i] == supply[i] + wrapped[i]
    // Since wrapped[i] will decrease below, outcome_escrowed[i] must also decrease
    coin_escrow::decrement_outcome_escrowed_for_all(escrow, false, amount);
    ...
    while (i < (outcome_count as u64)) {
        ...
        coin_escrow::decrement_wrapped_balance_pkg(escrow, i, false, amount);
        sub_from_balance_pkg(balance, (i as u8), false, amount);

        i = i + 1;
    };

    // Withdraw from escrow
    let stable_coin = coin_escrow::withdraw_stable_balance_pkg(escrow, amount, ctx);
    coin_escrow::decrement_user_backing_pkg(escrow, amount, false);
...
public fun recombine_balance_to_asset<AssetType, StableType>(
    ...
    // Invariant: outcome_escrowed[i] == supply[i] + wrapped[i]
    // Since wrapped[i] will decrease below, outcome_escrowed[i] must also decrease
    coin_escrow::decrement_outcome_escrowed_for_all(escrow, true, amount);
    ...
    while (i < (outcome_count as u64)) {
        ...
        // Decrement wrapped and update balance tracker
        coin_escrow::decrement_wrapped_balance_pkg(escrow, i, true, amount);
        sub_from_balance_pkg(balance, (i as u8), true, amount);

        i = i + 1;
    };

    // Withdraw from escrow
    let asset_coin = coin_escrow::withdraw_asset_balance_pkg(escrow, amount, ctx);
    coin_escrow::decrement_user_backing_pkg(escrow, amount, true);
