...
const EWindowStateInvalid: u64 = 18;
const EOracleNotInitialized: u64 = 19;
const EClockBeforeMarket: u64 = 20;
const EFutureTimestamp: u64 = 21;
...
/// Emitted when the conditional oracle writes a price observation.
/// This is for the futarchy TWAP oracle used by conditional AMM pools during proposals.
/// (The spot pool uses PCW_TWAP_oracle which emits WindowFinalized events instead.)
public struct PriceEvent has copy, drop {
    /// The oracle's object ID (can be used to identify which pool/outcome this is for)
    oracle_id: ID,
    /// Timestamp when this observation was recorded
    timestamp: u64,
    /// The last price written to the oracle (after capping)
    last_price: u128,
    /// Total cumulative price so far (for TWAP calculation: twap = cumulative / period)
    total_cumulative_price: u256,
}
...
public fun new_oracle(
    ...
    // Use checked multiplication to avoid overflow, clamping to u64::MAX
    let step_u128 = if (
        twap_cap_ppm > 0 && twap_initialization_price > (u128::max_value!() / (twap_cap_ppm as u128))
    ) {
        // Would overflow u128 multiplication, clamp to max u64
...
fun emit_price_event(oracle: &Oracle, timestamp: u64) {
    event::emit(PriceEvent {
        oracle_id: oracle.id.to_inner(),
        timestamp,
        last_price: oracle.last_price,
        total_cumulative_price: oracle.total_cumulative_price,
    });
}

// Called before swaps, LP events and before reading TWAP
// Requires &Clock to prevent future-timestamp attacks that would brick the oracle.
// If an attacker writes an observation at u64::MAX, all subsequent writes fail
// with ETimestampRegression because real clock < u64::MAX. This check blocks that.
public fun write_observation(oracle: &mut Oracle, timestamp: u64, price: u128, clock: &Clock) {
    assert!(timestamp <= clock.timestamp_ms(), EFutureTimestamp);
    write_observation_unchecked(oracle, timestamp, price);
}

/// Internal write_observation without clock validation.
/// MUST NOT be made public — use write_observation() which validates against clock.
fun write_observation_unchecked(oracle: &mut Oracle, timestamp: u64, price: u128) {
    // Sanity time checks
    ...
    if (timestamp == oracle.last_timestamp) {
        // Still update last_price to reflect current capped price, even though
        // no time-weighted contribution is added (Δt = 0 means no TWAP accumulation).
        // This keeps last_price fresh within the same checkpoint.
        ...
        emit_price_event(oracle, timestamp);
        return
    };
    ...
    if (oracle.last_timestamp <= delay_threshold && timestamp >= delay_threshold) {
        ...
        // This contribution is discarded by the reset below, so suppress PriceEvent
        // emission to keep event stream consistent with retained cumulative state.
        if (delay_threshold > oracle.last_timestamp) {
            twap_accumulate_internal(oracle, delay_threshold, price, false);
        };
...
fun twap_accumulate(oracle: &mut Oracle, timestamp: u64, price: u128) {
    twap_accumulate_internal(oracle, timestamp, price, true)
}

fun twap_accumulate_internal(
    oracle: &mut Oracle,
    timestamp: u64,
    price: u128,
    emit_events: bool,
    ...
    assert!(oracle.last_timestamp >= oracle.last_window_end, EWindowStateInvalid);
    ...
    if (duration_stage1 > 0) {
        ...
        intra_window_accumulation_internal(
    ...
    if (time_remaining_after_stage1 >= constants::twap_price_cap_window()) {
        ...
        multi_full_window_accumulation_internal(
            oracle, // Passes mutable reference, state will be updated
            price,
            num_full_windows,
...
fun intra_window_accumulation_internal(
    oracle: &mut Oracle,
    price: u128,
    additional_time_to_include: u64,
    timestamp: u64,
    ...
    if (time_since_last_window_end == constants::twap_price_cap_window()) {
        ...
        oracle.last_window_twap =
            ((oracle.total_cumulative_price - oracle.last_window_end_cumulative_price)
                / (constants::twap_price_cap_window() as u256)) as u128;
...
fun multi_full_window_accumulation_internal(
    oracle: &mut Oracle,
    price: u128,
    num_new_windows: u64, // N_W
    timestamp: u64,
...
public fun get_twap(oracle: &Oracle, clock: &Clock): u128 {
    ...
    // Time checks - order matters to avoid underflow before error check
    assert!(oracle.last_timestamp != 0, EOracleNotInitialized);
    assert!(current_time >= market_start_time_val, EClockBeforeMarket);
    // Use >= to be consistent with write_observation's accumulator reset logic
    // At exact equality, period=0 will trigger EZeroPeriod (need at least 1ms of data)
...
/// Get TWAP frozen at a specific target time instead of the current clock time.
/// Used to freeze TWAP at the scheduled trading deadline when computing governance outcomes.
/// Caller must have called write_observation(target_time, ...) first.
public fun get_twap_at(oracle: &Oracle, target_time: u64): u128 {
    assert!(oracle.market_start_time.is_some(), EMarketNotStarted);
    let market_start_time_val = *oracle.market_start_time.borrow();

    // REQUIRED: Caller must have called write_observation(target_time) first
    assert!(target_time == oracle.last_timestamp, EStaleTwap);
    assert!(oracle.last_timestamp != 0, EOracleNotInitialized);
    assert!(target_time >= market_start_time_val, EClockBeforeMarket);
    assert!(target_time - market_start_time_val >= oracle.twap_start_delay, ETwapNotStarted);

    let period = (target_time - market_start_time_val) - oracle.twap_start_delay;
    assert!(period > 0, EZeroPeriod);
...
/// RESTRICTED: Package-only to prevent unauthorized oracle manipulation.
/// Access control is enforced at the conditional_amm layer via MarketStateMutationAuth.
...
/// Returns the oracle's object ID (for event correlation)
public fun oracle_id(o: &Oracle): ID {
    o.id.to_inner()
}

/// Get full oracle state for debugging/monitoring (read-only, no mutations)
/// Returns all oracle fields needed to understand TWAP behavior
public fun get_full_state(
    oracle: &Oracle,
): (
    u128, // last_price
    u64, // last_timestamp
...
#[test_only]
/// Test-only write_observation without clock validation.
/// Production code MUST use write_observation() which validates timestamp <= clock.
public fun write_observation_for_testing(oracle: &mut Oracle, timestamp: u64, price: u128) {
    write_observation_unchecked(oracle, timestamp, price);
}
...
#[test_only]
/// Returns true if at least `min_period` milliseconds have elapsed since
/// the oracle's last observation. Does NOT validate TWAP readiness
/// (market start, delay, freshness) -- use get_twap() for that.
