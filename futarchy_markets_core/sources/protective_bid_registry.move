...
/// Protective Bid Registry - Enforce Max Active Bid Walls Per DAO
///
/// We store a vector of active bid records as managed data on the DAO `Account`,
/// capped at `constants::max_protective_bids_per_dao()`.
module futarchy_markets_core::protective_bid_registry;
...
const EProtectiveBidCapReached: u64 = 1;
const EExecutableAccountMismatch: u64 = 2;

// === Types ===

/// Key for Account managed-data.
public struct ProtectiveBidKey has copy, drop, store {}

/// Container stored on the DAO Account as managed data.
public struct ProtectiveBidRegistry has copy, drop, store {
    records: vector<ProtectiveBidRecord>,
}

/// Individual bid record.
public struct ProtectiveBidRecord has copy, drop, store {
    bid_id: ID,
    pool_id: ID,
}
...
public fun has_registry(account: &Account): bool {
    account::has_managed_data(account, ProtectiveBidKey {})
}

public fun bid_count(account: &Account, registry: &PackageRegistry): u64 {
    if (!has_registry(account)) return 0;
    let reg: &ProtectiveBidRegistry = account::borrow_managed_data_with_package_witness(
        account,
        registry,
        ProtectiveBidKey {},
        version::current(),
...
public fun bid_ids(account: &Account, registry: &PackageRegistry): vector<ID> {
    ...
    while (i < reg.records.length()) {
        ids.push_back(reg.records[i].bid_id);
        i = i + 1;
    };
    ids
}
...
/// Register a new active bid for this DAO. Enforces per-DAO cap.
...
public fun set_from_execution<Outcome: store, W: drop>(
    ...
    set_internal(account, registry, bid_id, pool_id);
}

/// Remove a specific bid record from a non-executable flow.
/// Intended for permissionless `protective_bid::close`.
public(package) fun clear_with_package_witness(
    account: &mut Account,
    registry: &PackageRegistry,
    bid_id: ID,
) {
    clear_internal(account, registry, bid_id);
}
...
fun set_internal(
    ...
    if (!has_registry(account)) {
        let reg = ProtectiveBidRegistry {
            records: vector[ProtectiveBidRecord { bid_id, pool_id }],
        };
        ...
        let reg: &mut ProtectiveBidRegistry =
        ...
        assert!(
            reg.records.length() < constants::max_protective_bids_per_dao(),
            EProtectiveBidCapReached,
        );
        reg.records.push_back(ProtectiveBidRecord { bid_id, pool_id });
    };
}
...
fun clear_internal(
    ...
    while (i < len) {
        if (reg.records[i].bid_id == bid_id) {
...
#[test_only]
public struct TestActionWitness has drop {}
...
#[test]
#[expected_failure(abort_code = EExecutableAccountMismatch)]
fun test_set_from_execution_rejects_executable_account_mismatch() {
    ...
    let mut account = account::new_for_testing(ctx);
    let test_clock = clock::create_for_testing(ctx);

    let params = intents::new_params(
        b"mismatch".to_string(),
        b"protective bid registry auth regression".to_string(),
        vector[1],
        2,
        &test_clock,
        ctx,
    );
    let intent = intents::new_intent_for_testing(
        params,
        TestOutcome {},
        @0xCAFE,
        TestIntentWitness {},
    ...
    let executable = executable::new_for_testing(intent, ctx);

    let bid_uid = object::new(ctx);
    let bid_id = object::uid_to_inner(&bid_uid);
    object::delete(bid_uid);
...
fun test_set_from_execution_accepts_current_action_witness() {
    ...
    assert!(bid_count(&account, &registry) == 1, 0);
