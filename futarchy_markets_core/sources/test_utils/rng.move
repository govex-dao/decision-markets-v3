...
public fun next_range(r: &mut Rng, lo: u64, hi_inclusive: u64): u64 {
    ...
    // Special case for full u64 range to avoid overflow
    if (lo == 0 && hi_inclusive == 0xFFFFFFFFFFFFFFFFu64) {
        return next_u64(r);
    };
...
public fun coin(r: &mut Rng, p_bps: u64): bool {
    assert!(p_bps <= 10000, 0);  // Ensure probability is valid [0, 10000] basis points
