#[test_only]
module futarchy_proposal::swap_core_tests;
...
/// Create a test escrow mutation registry with futarchy_markets_core authorized
#[test_only]
fun create_test_registry(ctx: &mut TxContext): EscrowMutationRegistry {
...
/// Add to balance and update tracking in escrow
/// This simulates the proper flow: deposit → mint (supply++) → wrap (supply--, wrapped++)
/// Net effect: escrowed++, supply stays 0, wrapped++
#[test_only]
fun add_to_balance_with_tracking(
    ...
    conditional_balance::add_to_balance(balance, outcome_idx, is_asset, amount, &escrow_auth);
    // Simulate deposit → mint → wrap:
    // 1. Deposit increases escrowed
    // 2. Mint + wrap is net zero for supply (mint ++ then wrap --)
    // 3. Wrap increases wrapped
    coin_escrow::increment_escrowed_for_testing(escrow, (outcome_idx as u64), is_asset, amount);
    coin_escrow::increment_wrapped_balance(escrow, (outcome_idx as u64), is_asset, amount, &escrow_auth);
}
...
fun add_liquidity_to_pools(
    ...
    // increment_supplies_for_all_outcomes updates both supply AND escrowed for all outcomes
    let escrow_auth = escrow_mutation_auth::create_for_testing();
...
fun create_test_proposal_trading(
    ...
    let mut proposal = proposal::new_for_testing<TEST_COIN_A, TEST_COIN_B>(
        ...
        0, // twap_start_delay
        option::some(500000u128), // twap_initial_observation
        100000, // twap_cap_ppm (10%)
        500000u128, // twap_threshold
...
fun test_finalize_swap_session_success() {
    ...
    let registry = create_test_registry(ctx);
    ...
    // Finalize session
    swap_core::finalize_swap_session(session, &mut escrow, &registry);
...
fun test_finalize_swap_session_wrong_market() {
    ...
    swap_core::finalize_swap_session(session, &mut escrow2, &registry);
...
fun test_swap_balance_asset_to_stable_success() {
    ...
    coin_escrow::destroy_for_testing(escrow);
    clock::destroy_for_testing(clock);
    escrow_mutation_auth::destroy_registry_for_testing(registry);
    ts::end(scenario);
}
...
fun test_swap_balance_asset_to_stable_insufficient_output() {
    ...
    conditional_balance::add_to_balance(&mut balance, 0, true, 10000, &balance_auth);
...
fun test_swap_balance_stable_to_asset_insufficient_output() {
    ...
    let mut balance = conditional_balance::new<TEST_COIN_A, TEST_COIN_B>(market_id, 2, ctx);
    let balance_auth = escrow_mutation_auth::create_for_testing();
    conditional_balance::add_to_balance(&mut balance, 0, false, 10000, &balance_auth);
