...
// ceil(a * b / c) using u256 intermediate. Returns u128.
public fun mul_div_mixed_up(a: u128, b: u64, c: u128): u128 {
    assert!(c != 0, EDivideByZero);
    let a_256 = (a as u256);
    let b_256 = (b as u256);
    let c_256 = (c as u256);
    let numerator = a_256 * b_256;
    let result = if (numerator == 0) {
        0
    } else {
        (numerator + c_256 - 1) / c_256
    };
    assert!(result <= (u128::max_value!() as u256), EOverflow);
    (result as u128)
}
...
public fun within_tolerance(a: u64, b: u64, tolerance_bps: u64): bool {
    if (tolerance_bps >= 10000) return true; // 100%+ tolerance = always within
    let diff = a.diff(b);
    let max_val = a.max(b);
